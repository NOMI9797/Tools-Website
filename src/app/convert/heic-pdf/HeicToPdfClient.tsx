...
export default function HeicToPdfClient() {
  ...
  const handleFileChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (invalidFiles.length > 0) {
      alert('Please select only HEIC image files');
      return;
    }
  ...
  return (
    <div className="bg-transparent">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Input Section */}
        <div>
          <h3 className="text-lg font-semibold text-gray-900 mb-6">Upload HEIC Images</h3>
          ...
          <div className="space-y-6">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <div className="text-gray-700">
                    {files.length > 0 ? `${files.length} HEIC file(s) selected` : "Click to select HEIC files"}
                  </div>
                  <div className="text-sm text-gray-600 mt-1">
                    Supports: HEIC format only (max 10MB each)
                  </div>
                </button>
              </div>
            </div>
            ...
            {files.length > 0 && (
              <div className="bg-gray-200/50 border border-gray-300/50 rounded-xl p-4 backdrop-blur-sm">
                <h4 className="font-semibold text-gray-900 mb-3">Selected HEIC Files ({files.length})</h4>
        ...
        <div>
          ...
          {convertedPdf ? (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-200/50 border border-gray-300/50 rounded-xl p-8 text-center backdrop-blur-sm">
                ...
                <p className="text-gray-700">
                  Your {files.length} HEIC image{files.length > 1 ? 's are' : ' is'} ready to be combined into a PDF.
                </p>
              </div>
            </div>
          ) : (
            <div className="bg-gray-200/50 border border-gray-300/50 rounded-xl p-8 text-center backdrop-blur-sm">
              ...
              <h4 className="text-lg font-semibold text-gray-900 mb-2">Ready to Convert</h4>
              <p className="text-gray-700">
                Select HEIC image files to start combining them into a PDF document.
              </p>
            </div>
          )}
        </div>
