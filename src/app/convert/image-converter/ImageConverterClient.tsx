...
export default function ImageConverterClient() {
  ...
  const handleFileChange = (selectedFile: File | null) => {
    ...
    // Simulate upload progress for consistent UX
    setIsUploading(true);
    setUploadProgress(0);
  ...
  async function handleConvert() {
    ...
    setIsLoading(true);
    setConvertProgress(0);
    // Simulate converting progress since HTTP upload doesn't expose progress here
    let current = 0;
    const interval = setInterval(() => {
      current += Math.random() * 15 + 5; // 5-20% increments
      if (current >= 95) {
        current = 95; // cap until response completes
      }
